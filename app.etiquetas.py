...
# Función para manejar la impresión desde la interfaz gráfica
def iniciar_impresion():
    tipo_etiqueta = tipo_combobox.get()
    inicio = inicio_entry.get().zfill(6)
    copias = int(copias_entry.get())
    
    url_base = "https://admin.misepis.com/code/"  # URL fija
    logo_path = "Logo.png"  # Asegúrate de que este sea el camino correcto del logo

    try:
        # Verificamos el valor del combobox para asignar "G" o "P"
        ...
        if size == "G":
            for i in range(copias):
                texto = str(int(inicio) + i).zfill(6)
                imprimir_texto_y_qr(texto, "", url_base, logo_path, size="G", single_copy=True)
        else:
            if copias == 1:
                imprimir_texto_y_qr(inicio, "", url_base, logo_path, size="P", single_copy=True)
            else:
                for i in range(0, copias, 2):
                    texto1 = str(int(inicio) + i).zfill(6)
                    texto2 = str(int(inicio) + i + 1).zfill(6)
                    imprimir_texto_y_qr(texto1, texto2, url_base, logo_path, size="P")
        messagebox.showinfo("Éxito", "Impresión completada")
    except Exception as e:
...
# Campos de entrada
tk.Label(root, text="Tipo de Etiqueta:").grid(row=0, column=0)
tipo_combobox = ttk.Combobox(root, values=["Grande", "Pequeña"], state="readonly")
tipo_combobox.grid(row=0, column=1)
tipo_combobox.current(0)  # Por defecto, seleccionar "Grande"

tk.Label(root, text="Número de Inicio:").grid(row=1, column=0)
inicio_entry = tk.Entry(root)
inicio_entry.grid(row=1, column=1)

tk.Label(root, text="Cantidad de Copias:").grid(row=2, column=0)
copias_entry = tk.Entry(root)
copias_entry.grid(row=2, column=1)

# Botón para iniciar la impresión
print_button = tk.Button(root, text="Imprimir", command=iniciar_impresion)
